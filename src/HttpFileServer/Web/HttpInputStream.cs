...
namespace HttpFileServer.Web
{
    ...
    internal class HttpRawUploadedContent : IDisposable
    {
        ...
        // contains data (either all of it or part read from file)
        private TempFile _file;

        private int _fileThreshold; // for sizes over this use file

        // content-length true when all data's in
        private int _length;

        #endregion Fields

        // length of the data temporary file with content (null when using byte[]) which part of
        // file is cached in data - offset which part of file is cached in data - length
        ...
        internal void AddBytes(byte[] data, int offset, int length)
        {
            ...
            if (_file == null)
            {
                ...
                // doesn't fit in _data but still under threshold possible if content-length is -1,
                // or when filtering
        ...
        internal byte[] GetAsByteArray()
        {
            // If the request is chunked, _data can be much larger than the actual number of bytes
            // read, and FillInFormCollection will call FillFromEncodedBytes and incorrectly append
            // a bunch of zeros to the last form value. Therefore, we copy the data into a smaller
            // array if _length < _data.Length
        ...
        private class TempFile : IDisposable
        {
            ...
            internal TempFile()
            {
                ...
                String tempDir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Templates), "HttpServerUploads");
            ...
            public void Dispose()
            {
                ...
                try
                {
                    // force filestream handle to close since we're using FILE_FLAG_DELETE_ON_CLOSE
